...
describe("workflowRuns aggregate total count query:", () => {
  ...
  it("Returns aggregate counts for each workflow", async () => {
    ...
    const aggregates: query_fedWorkflowRunsAggregateTotalCount_aggregate_items[] = response.data.fedWorkflowRunsAggregateTotalCount.aggregate;
    ...
    const longReadMngsAggregate = aggregates.find(aggregate => aggregate.groupBy?.workflowVersion?.workflow?.name === "long-read-mngs");
    expect(longReadMngsAggregate?.count).toBe(4);
    const benchmarkAggregate = aggregates.find(aggregate => aggregate.groupBy?.workflowVersion?.workflow?.name === "benchmark");
