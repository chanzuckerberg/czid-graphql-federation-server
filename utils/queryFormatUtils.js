...
exports.convertSequencingReadsQuery = exports.convertWorkflowRunsQuery = exports.formatFedQueryForNextGen = void 0;
var formatFedQueryForNextGen = function (query) {
    var cleanQuery = query;
    // remove fed Prefix
    var firstCurlyBracket = query.indexOf("{");
    var secondCurlyBracket = query.indexOf("{", firstCurlyBracket + 1);
    var fedIndex = query.indexOf("fed");
    if (fedIndex !== -1 &&
        ...
        var splitQueryOnFed = splitOnFirstFed[0].split("fed");
        var firstLetterLowerCase = splitQueryOnFed[1][0].toLowerCase();
...
exports.formatFedQueryForNextGen = formatFedQueryForNextGen;
var convertWorkflowRunsQuery = function (query) {
    return (query
        // Replace Fed variables.
        .replace(/query [\s\S]*?{/, "query ($where: WorkflowRunWhereClause, $orderBy: [WorkflowRunOrderByClause!]) {")
        // Remove fed prefix.
        ...
        .replace("entityInputs", "entityInputs(where: \n          { entityType: { _eq: \"sequencing_read\" }, inputEntityId: { _is_null: false } })"));
};
exports.convertWorkflowRunsQuery = convertWorkflowRunsQuery;
var convertSequencingReadsQuery = function (query) {
    // Remove fed prefix.
    ...
    if (/{\s*id\s*}/.test(query)) {
        return (query
            // Replace Fed variables.
            .replace(/query [\s\S]*?{/, "query ($where: SequencingReadWhereClause) {")
            // Replace Fed arguments.
    ...
    query = query
        ...
        // Replace Fed arguments.
        .replace("input: $input", "where: $where, orderBy: $orderBy, limitOffset: $limitOffset")
        // TODO: Make FE do this.
        // Add consensusGenomes filter (Mesh can't expose nested argument types?).
        .replace("consensusGenomes", "consensusGenomes(where: { producingRunId: { _in: $producingRunIds } })");
    for (var _i = 0, _a = [
        "nucleicAcid",
        "notes",
        "collectionLocation",
        ...
        var unsupportedField = _a[_i];
        query = query.replace(unsupportedField, "");
    }
    return query;
};
exports.convertSequencingReadsQuery = convertSequencingReadsQuery;
